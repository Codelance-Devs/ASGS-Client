...
const Navbar = () => {
	...
	return (
		<nav className='relative z-50 w-full bg-primaryBg border-b-primaryText border-b'>
			<div className='container mx-auto flex items-center justify-between border-b-primaryText px-4 py-6 text-primaryText lg:justify-start'>
				<ul className='hidden items-center gap-4 text-lg lg:flex'>
					{navOptions.map((option, index) => (
						...
						>
							<Link href={option.url} className={activeLink === option.url ? 'font-semibold underline' : ''}>{option.name}</Link>
						</li>
					))}
				</ul>
				<Link href={'/'} className='w-fit lg:mx-auto'>
					<h1 className='font-primaryFont text-xl font-semibold'>
						...
						<span className='font-secondaryFont font-semibold'>Supermarket</span>
					</h1>
				</Link>
			...
			>
				<ul className='flex flex-col gap-2 px-4 py-3 font-medium'>
					{navOptions.map((option, index) => (
						<li key={index} className='border-b border-primaryText/40 my-2'>
							<Link href={option.url} className={activeLink === option.url ? 'font-semibold' : ''}>{option.name}</Link>
						</li>
					))}
				</ul>
			</div>
